...
export default function CareerProfileWorkspace({ value, onChange }: Props) {
  ...
  const field = (key: string, label: string, placeholder = "", type = "text") => <div className="space-y-1.5"><Label htmlFor={`career-${key}`}>{label}</Label><Input id={`career-${key}`} type={type} value={text(editor?.draft[key])} onChange={(event) => update(key, event.target.value)} placeholder={placeholder} className="border-border bg-muted" /></div>;
  const textarea = (key: string, label: string, placeholder = "", hint?: string) => <div className="space-y-1.5"><Label htmlFor={`career-${key}`}>{label}</Label><Textarea id={`career-${key}`} value={text(editor?.draft[key])} onChange={(event) => update(key, event.target.value)} placeholder={placeholder} rows={3} className="border-border bg-muted leading-6" />{hint && <p className="text-xs text-muted-foreground">{hint}</p>}</div>;
  ...
  return <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/[0.09] via-card to-card shadow-card">
    ...
    <CardContent className="space-y-7 p-5 sm:p-6">
      ...
      <div className="grid gap-7 border-t border-border pt-7 lg:grid-cols-2">
        ...
        <section>
          ...
          {value.references.length ? <div className="space-y-3">{value.references.map((entry, index) => <div key={entry.id} className="rounded-xl border border-border bg-muted/50 p-3.5"><div className="flex justify-between gap-3"><div><p className="font-medium">{entry.fullName}</p><p className="mt-0.5 text-sm text-muted-foreground">{[entry.relationship, entry.company].filter(Boolean).join(" · ") || "Relationship not added"}</p>{entry.permissionToContact ? <p className="mt-2 flex items-center gap-1 text-xs text-emerald-400"><ShieldCheck className="h-3.5 w-3.5" /> Permission confirmed</p> : <p className="mt-2 text-xs text-amber-300">Not eligible for auto-fill</p>}</div><EntryActions onEdit={() => open("reference", index)} onDelete={() => remove("reference", index)} /></div></div>)}</div> : <EmptySection title="references" description="Only add a reference after you have permission to share their contact details." label="Add reference" onAdd={() => open("reference")} />}
        </section>
      </div>
    </CardContent>
    ...
    <Dialog open={Boolean(editor)} onOpenChange={(openState) => { if (!openState) setEditor(null); }}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto border-primary/20 bg-background p-5 text-foreground sm:p-6">
        ...
        {editor?.kind === "experience" && <div className="grid gap-4 sm:grid-cols-2">{field("title", "Job title", "Frontend Developer")}{field("company", "Company", "Company name")}{field("location", "Location", "City, Country")}{field("employmentType", "Employment type", "Full-time, contract…")} {field("startDate", "Start date", "", "month")} {editor.draft.isCurrent ? <div className="rounded-md border border-border bg-muted px-3 py-2 text-sm text-muted-foreground">Current role — no end date needed.</div> : field("endDate", "End date", "", "month")}<label className="col-span-full flex items-center gap-2 rounded-lg border border-border bg-muted px-3 py-2.5 text-sm"><input type="checkbox" checked={Boolean(editor.draft.isCurrent)} onChange={(event) => update("isCurrent", event.target.checked)} /> I currently work here</label><div className="col-span-full">{textarea("summary", "Role summary", "Describe your responsibilities and impact.")}</div><div className="col-span-full">{textarea("highlights", "Highlights", "One achievement per line", "Use real outcomes only.")}</div><div className="col-span-full">{textarea("skills", "Skills used", "React, TypeScript, …", "Separate skills with commas.")}</div></div>}
        {editor?.kind === "education" && <div className="grid gap-4 sm:grid-cols-2">{field("degree", "Degree", "BSc, MSc, Diploma…")}{field("institution", "Institution", "University name")}{field("fieldOfStudy", "Field of study", "Computer Science")}{field("location", "Location", "City, Country")}{field("startDate", "Start year", "2022", "text")}{field("endDate", "End year", "2026", "text")}<div className="space-y-1.5"><Label htmlFor="education-status">Status</Label><select id="education-status" value={text(editor.draft.status)} onChange={(event) => update("status", event.target.value)} className="flex h-10 w-full rounded-md border border-border bg-muted px-3 text-sm"><option value="">Not set</option><option value="Completed">Completed</option><option value="Ongoing">Ongoing</option></select></div>{field("grade", "Grade / GPA", "Optional")}<div className="col-span-full">{textarea("activities", "Activities or details", "Optional societies, thesis or honours")}</div></div>}
