...
export default function RecruiterCandidates() {
  ...
  return <DashboardLayout><div className="mx-auto max-w-5xl space-y-6">
    <section className="rounded-2xl border border-border bg-card p-6 shadow-card sm:p-8"><p className="text-xs font-semibold uppercase tracking-[.18em] text-primary">{t("recruiter.workspace")}</p><div className="mt-2 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="font-display text-3xl font-bold tracking-tight">{t("recruiter.applicantsTitle")}</h1><p className="mt-1 text-muted-foreground">{t("recruiter.applicantsCopy")}</p></div><Badge variant="outline" className="w-fit border-primary/25 bg-primary/10 px-3 py-1 text-primary">{t("recruiter.applicationsCount", { count: applications.length })}</Badge></div><div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4">{(Object.keys(statusLabel) as PipelineStatus[]).map(status => <div key={status} className="rounded-xl border border-border bg-muted/25 px-3 py-3"><p className="text-xl font-display font-semibold">{counts[status]}</p><p className="mt-0.5 text-xs text-muted-foreground">{statusLabel[status]}</p></div>)}</div></section>

    {loading ? <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : applications.length === 0 ? <Card className="border-dashed"><CardContent className="py-16 text-center"><UserRound className="mx-auto mb-3 h-9 w-9 text-muted-foreground" /><h2 className="font-display text-xl font-semibold">{t("recruiter.noApplicants")}</h2><p className="mt-2 text-sm text-muted-foreground">{t("recruiter.noApplicantsBody")}</p></CardContent></Card> : <div className="space-y-3">{applications.map(application => {
