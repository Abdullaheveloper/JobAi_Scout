...
const specs: Omit<AssistantTool, "execute">[] = [
  ...
  { name: "search_jobs", description: "Search jobs already stored in JobAI Scout using natural filters.", parameters: object({ filters: { type: "object", description: "Filters such as query, location, job_type, work_mode, experience_level, and limit", additionalProperties: true } }, ["filters"]), permission_tier: "safe", needs_screen_context: false },
  ...
  { name: "update_profile", description: "Update selected signed-in jobseeker profile fields after explicit confirmation.", parameters: object({ fields: { type: "object", description: "Profile fields and their new values", additionalProperties: true } }, ["fields"]), permission_tier: "confirm", needs_screen_context: false },
  ...
  { name: "set_notification_rule", description: "Create a notification rule for matching jobs or application events.", parameters: object({ criteria: { type: "object", description: "Notification matching criteria", additionalProperties: true } }, ["criteria"]), permission_tier: "safe", needs_screen_context: false },
  { name: "remember_user_preference", description: "Store a durable preference explicitly stated by the user. Use only for desired_role, skills, experience_level, location, or automation_preferences.", parameters: object({ key: { type: "string", enum: ["desired_role","skills","experience_level","location","automation_preferences"] }, value: { description: "The durable value to remember" } }, ["key","value"]), permission_tier: "safe", needs_screen_context: false },
];
