...
const specs: Omit<AssistantTool, "execute">[] = [
  ...
  { name: "scrape_jobs", description: "Explicitly start collecting fresh jobs from supported sources. Use only when the user clearly says Find Jobs, Start Scraping, or otherwise explicitly requests fresh collection; ordinary searches and filters must use search_jobs.", parameters: object({ query: string("Job search query"), source: string("Optional preferred source"), count: { type: "integer", minimum: 1, maximum: 25 } }, ["query"]), permission_tier: "safe", needs_screen_context: false },
