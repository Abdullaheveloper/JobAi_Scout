...
export async function collectLinkedInJobs(keywords: string[], locations: string[], maxItems = 25, filters: LinkedInFilters = {}, signal?: AbortSignal, onJobs?: (jobs: NormalizedJob[]) => void): Promise<NormalizedJob[]> {
  ...
  const items = await runApifyActor(actor, input, { signal, onItems: (partial) => onJobs?.(normalizeItems(partial)) });
