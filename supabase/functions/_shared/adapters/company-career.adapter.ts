...
export async function collectCompanyCareerJobs(url: string, company: string): Promise<NormalizedJob[]> {
  ...
  const response = await fetch("https://api.firecrawl.dev/v2/scrape", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ url, formats: [{ type: "json", schema, prompt: "Extract only current job openings from this official company careers page. Do not invent values." }], onlyMainContent: true }) });
