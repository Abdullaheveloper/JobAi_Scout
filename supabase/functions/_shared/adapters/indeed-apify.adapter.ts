...
export async function collectIndeedJobs(query: string, location: string, maxItems = 10): Promise<NormalizedJob[]> {
  const actor = Deno.env.get("APIFY_INDEED_ACTOR"); if (!actor) throw new Error("APIFY_INDEED_ACTOR is missing");
  const items = await runApifyActor(actor, { position: query, location, country: "PK", maxItemsPerSearch: maxItems, saveOnlyUniqueItems: true, parseCompanyDetails: false, followApplyRedirects: false });
