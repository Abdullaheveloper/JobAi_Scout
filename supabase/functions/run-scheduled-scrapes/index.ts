 ...
 * after the Automation usage limit check short-circuits when capped.
 */
Deno.serve(async (req) => {
  ...
  await Promise.allSettled(claimed.map(async (schedule) => {
    // Limit check FIRST — never run scrape/match when capped.
    ...
    if (!guarded.allowed) {
      const { error: statusError } = await admin
        ...
        .update({ last_run_status: "skipped_limit" })
        .eq("id", schedule.id);
      if (statusError) {
        console.error("[run-scheduled-scrapes] limit status update failed", schedule.id, statusError.message);
      }
      console.warn(
        "[run-scheduled-scrapes] usage limit blocked",
        schedule.user_id,
        guarded.body.error,
      );
      return;
