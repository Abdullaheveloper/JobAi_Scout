-- Broaden Browse Jobs while keeping matching and career-level filtering server-side.
ALTER TABLE public.job_scrape_sessions
  DROP CONSTRAINT IF EXISTS job_scrape_sessions_session_status_check;

ALTER TABLE public.job_scrape_sessions
  ADD CONSTRAINT job_scrape_sessions_session_status_check
  CHECK (session_status IN ('pending', 'running', 'completed', 'partially_completed', 'failed', 'stopped'));

DROP POLICY IF EXISTS "Users can stop own job scrape sessions" ON public.job_scrape_sessions;
CREATE POLICY "Users can stop own job scrape sessions"
  ON public.job_scrape_sessions FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);
GRANT UPDATE ON public.job_scrape_sessions TO authenticated;
